...
El método de Ziegler & Nichols, en su variante de "ciclo último" (también conocido como método de "oscilación"), es una técnica empírica de sintonización de controladores PID en lazo cerrado, desarrollada por John G. Ziegler y Nathaniel B. Nichols en 1942. Su simplicidad operativa ha consolidado su popularidad, aunque requiere la inducción de un estado de oscilación sostenida en el sistema, lo cual puede implicar una cierta perturbación del proceso.
...
Para ilustrar la aplicación de este método de forma analítica, consideremos un sistema con la función de transferencia de lazo abierto:

$$G(s) = \frac{1}{s(s+1)(s+2)}$$

Esta función de transferencia corresponde a un sistema de tercer orden. Para aplicar el método de Ziegler & Nichols (ciclo último), se asume un controlador puramente proporcional $C(s) = K_p$. La función de transferencia en lazo cerrado $G_o(s)$ (considerando una realimentación unitaria) se define como:

$$G_o(s) = \frac{C(s)G(s)}{1 + C(s)G(s)} = \frac{K_p \cdot \frac{1}{s(s+1)(s+2)}}{1 + K_p \cdot \frac{1}{s(s+1)(s+2)}}$$

Simplificando el denominador, que es el polinomio característico de lazo cerrado:

$$s(s+1)(s+2) + K_p = 0$$$$s(s^2 + 3s + 2) + K_p = 0$$$$s^3 + 3s^2 + 2s + K_p = 0$$
...
Ahora, agrupamos los términos reales e imaginarios de la ecuación:
...
$$2\omega - \omega^3 = 0$$Factorizamos $\omega$:$$\omega(2 - \omega^2) = 0$$
Esta ecuación tiene tres soluciones: $\omega = 0$, $\omega = \sqrt{2}$, y $\omega = -\sqrt{2}$. La solución $\omega = 0$ corresponde al estado estacionario y no a una oscilación. Las soluciones no triviales para la frecuencia de oscilación son $\omega_u = \sqrt{2}$ rad/s. Este valor representa la frecuencia última de oscilación.

**Resolviendo para la Parte Real:**
$$K_p - 3\omega^2 = 0$$Sustituimos el valor de $\omega^2 = 2$ (derivado de $\omega_u = \sqrt{2}$):$$K_p - 3(2) = 0$$$$K_p - 6 = 0$$$$K_p = 6$$

Este valor de $K_p$ es la **ganancia última** ($K_u = 6$). Es la ganancia proporcional que lleva al sistema al borde de la estabilidad, resultando en oscilaciones sostenidas.

Con la frecuencia última de oscilación $\omega_u = \sqrt{2}$ rad/s, el **periodo último** ($P_u$) se calcula como:

$$P_u = \frac{2\pi}{\omega_u} = \frac{2\pi}{\sqrt{2}} = \pi\sqrt{2} \approx 4.4428 \text{ segundos}$$

Finalmente, se utilizan los valores de $K_u = 6$ y $P_u \approx 4.4428$ segundos en la Tabla 1, la cual resume los coeficientes de sintonización de Ziegler & Nichols para diferentes tipos de controladores:
...
* $T_i = \frac{P_u}{2} = \frac{4.4428}{2} = 2.2214$ segundos
* $T_d = \frac{P_u}{8} = \frac{4.4428}{8} = 0.55535$ segundos

Por lo tanto, los parámetros para el controlador PID, sintonizado mediante el método de Ziegler & Nichols (ciclo último), son: $K_p = 3.6$, $T_i = 2.2214$ s, y $T_d = 0.55535$ s. Estos valores proporcionan una base sólida para la implementación del controlador, que podría requerir un ajuste fino posterior en el sistema real.
...
1.  **Preparación del Sistema:** El sistema se configura en lazo cerrado, pero en lugar de un controlador PID sintonizado, se introduce un relé como elemento de control. Es recomendable medir el tiempo estacionario del sistema en lazo abierto antes de la prueba, para tener una referencia del comportamiento intrínseco.
...
4.  **Extracción de Parámetros de Oscilación:** De la oscilación sostenida de la salida del sistema, se miden dos parámetros clave:
    * El **periodo último ($P_u$)**: El tiempo que tarda una oscilación completa.
    * La **amplitud del ciclo último ($A_u$)**: La mitad de la diferencia entre el valor máximo y mínimo de la oscilación de la salida de la planta.
5.  **Cálculo de la Ganancia Crítica Equivalente ($K_c$):** La ganancia crítica equivalente ($K_c$) se determina a partir de la amplitud de la señal de salida del relé ($d$), la amplitud de la oscilación de la planta ($A_u$), y la histéresis del relé ($\epsilon$). La fórmula que relaciona estos parámetros se deriva del análisis de funciones de descripción (DF - Describing Function) para elementos no lineales:
    ...
    Es fundamental que durante la prueba, se logre un desfase de $180^\circ$ entre la señal del relé y la fundamental de la oscilación de la salida de la planta para que esta fórmula sea válida. En sistemas puramente lineales, $K_c$ correspondería a la ganancia que llevaría el sistema a oscilación sostenida en el método de Ziegler & Nichols.
6.  **Determinación de la Ganancia Estática (opcional):** Aunque no siempre es estrictamente necesario para la sintonización, la ganancia estática del sistema puede ser estimada a partir del ensayo con el relé, lo que proporciona información adicional sobre el comportamiento a largo plazo del proceso.

A partir de los valores obtenidos de $P_u$ y $K_c$, se utilizan ecuaciones de sintonización específicas, a menudo adaptadas de las reglas de Ziegler & Nichols o de otras metodologías de desempeño. Para la sintonización de un controlador PI con el método del Relé, buscando un sobreimpulso máximo inferior al 10%, se pueden emplear las siguientes fórmulas, que son una adaptación de las reglas de Ziegler & Nichols para un desempeño más robusto:
...
💡**Ejemplo 4:** El siguiente diagrama de bloques ilustra la topología de un sistema de control donde se ha implementado un relé para la prueba de sintonización.
...
En este esquema, la señal de error ($e(t)$) es la entrada al bloque del relé, cuya salida ($u(t)$) constituye la señal de control aplicada a la planta. El relé, con su característica no lineal de histéresis, alterna su salida entre dos valores discretos en función del cruce del error por los umbrales de conmutación. Esta acción conmutada induce una oscilación en la variable de salida del proceso, la cual es esencial para la identificación de los parámetros.
...
💡**Ejemplo 6:** La siguiente figura ilustra el impacto del fenómeno *wind-up* en la respuesta de un sistema real.
...
La función de transferencia de la planta se puede expandir para obtener un polinomio en el denominador:
$$G(s) = \frac{1}{s(s+1)(s+2)} = \frac{1}{s(s^2 + 3s + 2)} = \frac{1}{s^3 + 3s^2 + 2s}$$
...
$$G_o(s) = \frac{C(s)G(s)}{1 + C(s)G(s)} = \frac{K_p \cdot \frac{1}{s^3 + 3s^2 + 2s}}{1 + K_p \cdot \frac{1}{s^3 + 3s^2 + 2s}}$$
...
$$(j\omega)^3 + 3(j\omega)^2 + 2(j\omega) + K_p = 0$$Recordando que $j^2 = -1$ y $j^3 = -j$:$$-j\omega^3 - 3\omega^2 + j2\omega + K_p = 0$$
...
* $2 - \omega^2 = 0 \implies \omega^2 = 2 \implies \omega = \pm\sqrt{2}$

La frecuencia de oscilación no trivial es $\omega_u = \sqrt{2}$ rad/s. Este valor es la **frecuencia última de oscilación**.

**2. Igualando la Parte Real a Cero:**
$$K_p - 3\omega^2 = 0$$Ahora, sustituimos el valor de $\omega^2 = 2$ que obtuvimos de la parte imaginaria:$$K_p - 3(2) = 0$$$$K_p - 6 = 0$$$$K_p = 6$$

Este valor de $K_p$ es la **ganancia última** ($K_u = 6$). Es la ganancia que lleva al sistema al umbral de la estabilidad, resultando en oscilaciones sostenidas.

Con la frecuencia última de oscilación $\omega_u = \sqrt{2}$ rad/s, el **periodo último** ($P_u$) se calcula como el inverso de la frecuencia angular dividido por $2\pi$:
$$P_u = \frac{2\pi}{\omega_u} = \frac{2\pi}{\sqrt{2}} = \pi\sqrt{2} \approx 4.4428 \text{ segundos}$$

Finalmente, se utilizan los valores de $K_u = 6$ y $P_u \approx 4.4428$ segundos en la Tabla 1 para determinar los parámetros del controlador PID. Según las reglas de Ziegler & Nichols para un controlador PID:

* **Ganancia Proporcional ($K_p$):**
    $K_p = 0.6 \times K_u = 0.6 \times 6 = 3.6$

* **Tiempo Integral ($T_i$):**
    $T_i = \frac{P_u}{2} = \frac{4.4428}{2} = 2.2214 \text{ segundos}$

* **Tiempo Derivativo ($T_d$):**
    $T_d = \frac{P_u}{8} = \frac{4.4428}{8} = 0.55535 \text{ segundos}$

Por lo tanto, los parámetros del controlador PID sintonizado por el método de Ziegler & Nichols son: $K_p = 3.6$, $T_i = 2.2214$ s, y $T_d = 0.55535$ s.
...
* **Eficiencia de la Sintonización en Lazo Cerrado:** Las metodologías de sintonización en lazo cerrado ofrecen una aproximación práctica y altamente relevante para la industria. Permiten la caracterización dinámica precisa de un sistema bajo sus condiciones operativas normales, lo cual es ventajoso en entornos donde la interrupción del proceso para pruebas en lazo abierto es inviable o costosa. Esto contribuye a una sintonización más representativa y, por ende, a un desempeño de control superior.
* **Pertinencia del Método de Ziegler & Nichols:** A pesar de su antigüedad, el método de Ziegler & Nichols en su variante de ciclo último sigue siendo una herramienta empírica válida y ampliamente aplicada. Proporciona un punto de partida accesible para obtener una sintonización inicial razonable de los controladores PID, siendo especialmente útil en la ausencia de modelos precisos del sistema.
* **Ventajas del Método del Relé:** El Método del Relé se consolida como una técnica de sintonización superior, al ofrecer una forma más controlada y menos intrusiva de inducir las oscilaciones necesarias para la identificación de parámetros. Su capacidad para evitar la saturación excesiva de la señal de control y su utilidad como base para sistemas de autosintonización lo posicionan como una herramienta de mayor flexibilidad y seguridad en comparación con Ziegler & Nichols, especialmente en aplicaciones industriales.
* **Manejo Crítico del Fenómeno *Wind-up*:** La consideración y mitigación del fenómeno de acumulación integral (*wind-up*) es una fase crítica en el diseño e implementación de controladores PI y PID en lazo cerrado. Es imperativo integrar estrategias anti-*wind-up* (como la saturación integral, integración condicional o, preferentemente, el recálculo y seguimiento). Ignorar este fenómeno puede llevar a un desempeño degradado del sistema, caracterizado por sobreimpulsos excesivos, tiempos de asentamiento prolongados y comportamientos oscilatorios indeseados, comprometiendo la estabilidad y eficiencia del control.
...
* Aström, K.J. y T. Hägglund. "Benchmark Systems for PID Control". IFAC Workshop on Digital Control: Past, Present and Future of PID Control, Terrassa, España, Abril 5-7, 2000.
